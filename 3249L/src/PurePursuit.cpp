...
const float Paths[PathLimit][2] = {
    ...
    {2,7}
};//erm actually these are points in a path and not the paths themselves
float BestPoint[1];
bool Finished = false;
...
//goal point is the position on the line thats the closest to the look ahead distance
    ...
    MaxPoint[1] = std::max(y[0],y[1]);
        ...
        if (Colliding && /*X((MinPoint[0] <= Sol1[0] <= MaxPoint[0])||(MinPoint[0] <= Sol1[1] <= MaxPoint[0])) && /*Y((MinPoint[1] <= Sol2[0] <= MaxPoint[1])||(MinPoint[1] <= Sol2[1] <= MaxPoint[1]))){// check if its within the range of the bounds
            return true;
        }
}

    bool GetBestPoint(){//trying to find which of the two points we should go after, otherwise itll send us backwards
    float Distance[1];//for getting the distance from the end point to the solution point
    float x[1];
    float y[1];
    for(int i = PathLimit; i > 0; i--){//ooooo scary inverse for loop. I know i will regret it later but it might be helpful
        ...
        y[1] = Paths[i][1];
        if (CheckForIntersection(x,y)){// if a valid intercetion happens, then our job here is done
            Distance[0] = x[1] - Sol1[0];
            if (abs(Sol1[0] - x[1]) < abs(Sol1[1] - x[1]) || abs(Sol2[0] - x[1]) < abs(Sol2[1] - x[1])){//if the first solution is smaller, or closer to the goal, then the best distance is the first distance. 
                Distance[1] = Sol2[0];
                Distance[0] = Sol1[0];
                BestPoint[0] = Distance[0]; BestPoint[1] = Distance[1]; 
                break;
            }else{
                Distance[1] = Sol2[1];
                Distance[0] = Sol1[1];
